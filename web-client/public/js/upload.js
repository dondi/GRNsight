/* globals sliderObject, sliderGroupController, drawGraph */
...
$(function () {
    ...
    // Setup some Variables that will remember information about the uploaded graph
    var currentNetwork = null;
    var reloader = function () { };

    // Setup Sliders and Slider Labels
    ...
    var chargeDistSlider = new sliderObject(CHARGE_DIST_SLIDER_ID, CHARGE_DIST_VALUE, CHARGE_DIST_DEFAULT, false);
    var gravitySlider = new sliderObject(GRAVITY_SLIDER_ID, GRAVITY_VALUE, GRAVITY_DEFAULT, true);
    var sliders = new sliderGroupController([linkDistanceSlider, chargeSlider, chargeDistSlider, gravitySlider]);
    sliders.setSliderHandlers();
    sliders.updateValues();
    sliders.configureSliderControllers();

    // Setup Edge Color Preferences
    ...
    // Setup "Select New File" button on warnings/error modal
    // Remember what file type was responsible for the error, and allow that type to be uploaded again.
    var uploadIds = [ "#upload-sif", "#upload-graphml", "#upload" ];
    ...
    // Error Modal Handler
    var networkErrorDisplayer = function (xhr, status, error) { // eslint-disable-line no-unused-vars
    ...
    // Warning Modal Handler.. Can probably be condensed
    ...
    // The next few functions are where the magic happens...
    // The data is prepped and sent to drawGraph
    ...
    var performExport = function (route, extension, sheetType) {
        return function () {
            if (!$(this).parent().hasClass("disabled")) {
                ...
                var networkFilename =
                    filenameWithExtension(sheetType !== currentNetwork.sheetType ? sheetType : "", extension);
