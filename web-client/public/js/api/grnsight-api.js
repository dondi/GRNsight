...
const buildExpressionURL = function(selection, genes) {
  const baseQuery = `expressiondb?dataset=${
    selection.dataset
  }&genes=${buildExpressionGeneQuery(genes)}`;
...
const responseExpressionData = (formData, queryURL) => {
  return new Promise(function(resolve) {
    ...
    (formData
      ...
      .error(
        console.log(
          "Error in accessing expression database. Result may just be loading."
        )
      );
  });
};
...
const responseNetworkData = (formData, queryURL) => {
  return new Promise(function(resolve) {
    ...
    (formData
      ...
      .error(
        console.log(
          "Error in accessing network database. Result may just be loading."
        )
      );
  });
};
...
const uploadCustomWorkbook = (workbook, grnState) => {
  let queryURL = buildCustomWorkbookURL(
    workbook.name,
    workbook.genes,
    workbook.links
  );
