...
var updateSliderDisplayedValue = function (slider, element) {
    ...
    $(slider.valueId).html(value +
        ((slider.needsAppendedZeros && (value.length === GRAVITY_LENGTH_WITHOUT_ZERO)) ? "0" : ""));
...
var sliderGroupController = function (sliderArray) {
    ...
    this.updateValues = function () {
        for (var i = 0; i < this.numberOfSliders; i++) {
            ...
            $(this.sliders[i].valueId).html(this.sliders[i].currentVal + ((this.sliders[i].needsAppendedZeros
                && this.sliders[i].currentVal.toString().length === GRAVITY_LENGTH_WITHOUT_ZERO) ? "0" : ""));
        }
    };
