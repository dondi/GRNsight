...
const updateViewportSize = (currentValue) => {
  ...
  const fitContainer = (dimensions) => {
    const fitWidth = dimensions
      ? dimensions.width - WIDTH_OFFSET
      : container.width();
...
const updateSliderState = (slidersLocked) => {
  const forceGraphDisabled =
    grnState.graphLayout === GRID_LAYOUT || slidersLocked;
...
const updateChargeSliderValues = () => {
  ...
  $(CHARGE_SLIDER_SIDEBAR).html(
    grnState.chargeSlider.currentVal +
      ...
      grnState.chargeSlider.currentVal.toString().length ===
        GRAVITY_LENGTH_WITHOUT_ZERO
        ? "0"
        : "")
  );
...
export const hasExpressionData = (sheets) => {
  return Object.keys(sheets).some((property) =>
    property.match(ENDS_IN_EXPRESSION_REGEXP)
  );
};
...
const resetDatasetDropdownMenus = (workbook) => {
  ...
  $(BOTTOM_DATASET_SELECTION_SIDEBAR).append(
    $("<option>")
      .attr("value", "Same as Top Dataset")
      .text("Same as Top Dataset")
  );
  ...
  const addOptionsToDropdown = (options, groupLabel) => {
    const topOptgroup = $("<optgroup>").attr("label", groupLabel);
    const bottomOptgroup = $("<optgroup>").attr("label", groupLabel);
    const datasetValues = options.map((o) => o.value).filter(Boolean);
    datasetValues.forEach((name) => {
      const shortenedName = shortenExpressionSheetName(name);
      const topOption = $("<option>")
        .addClass("dataset-option")
        .attr("value", name)
        .text(shortenedName);
      const bottomOption = $("<option>")
        .addClass("dataset-option")
        .attr("value", name)
        .text(shortenedName);
  ...
  $("#topDatasetDropdownMenu li a span")
    .first()
    .addClass("glyphicon-ok");
  $("#bottomDatasetDropdownMenu li a span")
    .first()
    .addClass("glyphicon-ok");
};

const updateLogFoldChangeMaxValue = () => {
  var value = logFoldChangeMaxValueInputValidation(
...
const updateBottomDataset = () => {
  if (grnState.nodeColoring.bottomDataSameAsTop) {
    ...
    $(
      `${BOTTOM_DATASET_SELECTION_MENU} li[value='${"Same as Top Dataset"}'] a span`
    ).addClass("glyphicon-ok");
  } else {
    $(BOTTOM_DATASET_SELECTION_SIDEBAR).val(
      grnState.nodeColoring.bottomDataset
    );
    removeAllChecksFromMenuDatasetOptions(BOTTOM_DATASET_SELECTION_MENU);
...
export const updateApp = (grnState) => {
  if (grnState.newWorkbook) {
    ...
    grnState.normalizationMax = max(
      grnState.workbook.positiveWeights.concat(
        grnState.workbook.negativeWeights
      )
    );
  ...
  if (
    ...
    if (
      ...
      grnState.workbook.expression[grnState.nodeColoring.topDataset] ===
        undefined
    ) {
  ...
  if (
    grnState.workbook !== null &&
    grnState.workbook.sheetType === "weighted"
  ) {
    showEdgeWeightOptions();
  } else if (
    grnState.workbook !== null &&
    grnState.workbook.sheetType === "unweighted"
  ) {
    hideEdgeWeightOptions();
  } else {
    hideEdgeWeightOptions();
