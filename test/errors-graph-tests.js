...
describe("errors-graph-tests", function () {
    ...
    describe("incorrect-missing-data-error-10-genes-0-edges", function () {
        it("should not return any errors", function () {
            test.noErrors("test-files/graph-tests/different-sized-workbooks/10-genes-0-edges.xlsx");
        });
    });
    ...
    describe("over-75-genes-or-150-nodes", function () {
        it("should return invalid workbook size error", function () {
            this.timeout(10000);
            test.workbookSizeError("test-files/graph-tests/different-sized-workbooks/80-genes-0-edges.xlsx", 1);
            test.workbookSizeError("test-files/graph-tests/different-sized-workbooks/45-genes-max-edges.xlsx", 1);
        });
    });

    describe("over-50-genes-or-100-nodes", function () {
        it("should return invalid workbook size warning", function () {
            test.invalidworkbookSizeWarning("test-files/graph-tests/different-sized-workbooks/52-genes-0-edges.xlsx", 1);
            test.invalidworkbookSizeWarning("test-files/graph-tests/different-sized-workbooks/34-genes-111-edges.xlsx", 1);
        });
    });
