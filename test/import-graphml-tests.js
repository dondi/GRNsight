...
describe("Import from GraphML", function () {
    it("should import unweighted workbooks from GraphML correctly", function () {
        expect(importController.graphMlToGrnsight(unweightedTestGraphMl)).to.deep.equal(
            expectedUnweightedWorkbook
        );
    });

    it("should import weighted workbooks from GraphML correctly", function () {
        expect(importController.graphMlToGrnsight(weightedTestGraphMl)).to.deep.equal(
            expectedWeightedWorkbook
        );
    });

    it("should import unweighted workbooks with cycles from GraphML correctly", function () {
        expect(importController.graphMlToGrnsight(unweightedTestGraphMlWithCycle)).to.deep.equal(
            expectedUnweightedWorkbookWithCycle
        );
    });

    it("should import weighted workbooks with cycles from GraphML correctly", function () {
        expect(importController.graphMlToGrnsight(weightedTestGraphMlWithCycle)).to.deep.equal(
    ...
    it("should issue an general graphML syntax error because there is a missing end tag", function () {
        expect(importController.graphMlToGrnsight(missingEndTagTestGraphMl).errors.length).to.equal(
            1
        );
    ...
    it("should issue a warning if edgedefault is not set to 'directed'", function () {
        var undirected = unweightedTestGraphMl.replace(
            'edgedefault="directed"',
            'edgedefault="undirected"'
        );
        expect(importController.graphMlToGrnsight(undirected).warnings).to.deep.equal([
            constants.warnings.EDGE_DEFAULT_NOT_DIRECTED,
        ]);

        var noEdgedefault = unweightedTestGraphMl.replace('edgedefault="directed"', "");
        expect(importController.graphMlToGrnsight(noEdgedefault).warnings).to.deep.equal([
            constants.warnings.EDGE_DEFAULT_NOT_DIRECTED,
        ]);
    });
    ...
    it("should issue a warning if a weighted graph has edges without weights", function () {
        ...
        expect(importController.graphMlToGrnsight(missingWeight).warnings).to.deep.equal([
            constants.warnings.EDGES_WITHOUT_WEIGHTS,
        ]);

        var nanWeight = weightedTestGraphMl.replace(
            '<data key="edge-value-id">0.5</data>',
            '<data key="edge-value-id">pizza pizza</data>'
        );
        expect(importController.graphMlToGrnsight(nanWeight).warnings).to.deep.equal([
            constants.warnings.EDGES_WITHOUT_WEIGHTS,
        ]);
    });
    ...
    it("should ignore unsupported GraphML features", function () {
        fs.readFile(
            __dirname + "/../test-files/import-samples/hyper.graphml",
            UTF8,
            function (error, data) {
                expect(importController.graphMlToGrnsight(data)).to.deep.equal(
                    initWorkbook({
                        genes: [
                            { name: "n0" },
                            { name: "n1" },
        ...
        fs.readFile(
            __dirname + "/../test-files/import-samples/nested.graphml",
            UTF8,
            function (error, data) {
                expect(importController.graphMlToGrnsight(data)).to.deep.equal(
                    initWorkbook({
    ...
    it("should read labels from keys if available", function () {
        fs.readFile(
            __dirname +
                "/../test-files/import-samples/4-gene_4-edge_Manual-Cytoscape_test-naming.graphml",
            UTF8,
            function (error, data) {
                expect(importController.graphMlToGrnsight(data)).to.deep.equal(
                    initWorkbook({
    ...
    it("should read text labels from yED keys if available", function () {
        fs.readFile(
            __dirname + "/../test-files/import-samples/graph-with-yed-tags.graphml",
            UTF8,
            function (error, data) {
                expect(importController.graphMlToGrnsight(data)).to.deep.equal(
                    initWorkbook({
                        genes: [{ name: "January" }, { name: "n1" }],
    ...
    it("should read node label objects from yED keys if available", function () {
        fs.readFile(
            __dirname + "/../test-files/import-samples/4-node_4-edge_manual-yED.graphml",
            UTF8,
            function (error, data) {
                expect(importController.graphMlToGrnsight(data)).to.deep.equal(
                    initWorkbook({
    ...
    it("should import this weighted workbook from GraphML correctly", function () {
        expect(importController.graphMlToGrnsight(templateGraphmlFileForNewTests)).to.deep.equal(
            newExpectedWeightedWorkbook
        );
    });
    ...
    it("should issue a missing end tag because there there is a missing graph open tag", function () {
        expect(importController.graphMlToGrnsight(missingGraphOpenTag).errors.length).to.equal(1);
    ...
    it("should issue an incorrect closing tag error because there is a '</' instead of '</graph>'", function () {
        expect(importController.graphMlToGrnsight(incompleteClosingTag).errors.length).to.equal(1);
    ...
    it("should issue an unclosed root tag when the </graphml> closing tag is missing", function () {
        expect(importController.graphMlToGrnsight(unclosedRootTag).errors.length).to.equal(1);

        expect(importController.graphMlToGrnsight(unclosedRootTag).errors[0].errorCode).to.equal(
            "GRAPHML_MISSING_GRAPHML_CLOSE_TAG"
        );
    });

    it("should issue unpaired quotation mark error because there is a missing end quote", function () {
        expect(importController.graphMlToGrnsight(unpairedQuote).errors.length).to.equal(1);

        expect(importController.graphMlToGrnsight(unpairedQuote).errors[0].errorCode).to.equal(
            "GRAPHML_UNPAIRED_QUOTE"
        );
    });

    it("should issue an invalid character error because there is an invalid character somewhere", function () {
        expect(importController.graphMlToGrnsight(invalidCharacterInName).errors.length).to.equal(
            1
        );
    ...
    it("should issue an unencoded tag error because there are extra tags at the end of a line", function () {
        expect(importController.graphMlToGrnsight(unencodedTag).errors.length).to.equal(1);

        expect(importController.graphMlToGrnsight(unencodedTag).errors[0].errorCode).to.equal(
            "GRAPHML_UNENCODED_TAG"
        );
    });
    ...
    it("should issue missing opening quote error because there is a missing opening quote", function () {
        expect(importController.graphMlToGrnsight(missingOpeningQuote).errors.length).to.equal(1);
