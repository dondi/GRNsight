// Each strain gets its own sheet in the workbook
  // Potential errors:
// Each sheet named "STRAINNAME_log2_expression"
  // X Wrong sheet naming format
// Left column: id. 3 letters and a number generally (?)
  // X Incorrect name/wrong naming format
// All other column headers are whole numbers. (Timepoints) (Only 15, 30, and/or 60)
  // X Non 15 or 30 or 60 number
  // X Not a number
// Same timepoints for all strains...But # replications may vary.
  // X Different timepoints (one only has 15, another only has 30)
  // X Make sure it accepts different # replications
  ...
  describe("Import from Excel", function () {
      it("should import unweighted networks from Excel correctly", function () {
          expect(
            ...
            // bc it doesn't require an importer...It's already in the right format, correct?
            unweightedTestExcel
...
//
      ...
      it("should issue a warning if a weighted graph has edges without weights", function () {
          // import the Excel with missing weight example
          // var missingWeight = path to the desired excel file
          expect(
              missingWeight.warnings
          ).to.deep.equal([ constants.warnings.EDGES_WITHOUT_WEIGHTS ]);

          // var nanWeight = path to desired excel file ;
          expect(
              nanWeight.warnings
          ).to.deep.equal([ constants.warnings.EDGES_WITHOUT_WEIGHTS ]);
      });
