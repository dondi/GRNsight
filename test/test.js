...
function specialCharacterError(input, frequency) {
  var sheet = xlsx.parse(input);
  var network = spreadsheetController.parseSheet(sheet);
...
function warningsCountError(input, frequency) {
  var sheet = xlsx.parse(input);
  var network = spreadsheetController.parseSheet(sheet);
  var warningsCountErrorArray = network.errors.filter(function(x){return x.errorCode === "WARNINGS_OVERLOAD";});
...
function errorsCountError(input, frequency) {
  var sheet = xlsx.parse(input);
  var network = spreadsheetController.parseSheet(sheet);
  var errorsCountErrorArray = network.errors.filter(function(x){return x.errorCode === "ERRORS_OVERLOAD"});
