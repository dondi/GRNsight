...
var sheet = xlsx.parse(
    __dirname + "/dahlquist_wt-data_21-gene-_sample-output_20140122_est_out_1.xlsx"
);
...
var expectedSheet = {
    worksheets: [
        ...
        { name: "Sheet2", data: [["rdsdfgrs", "gsdfgdsfg"], ["sdfgdsf"]] },
    ],
};
            ...
            for (var i = 0; i < testParse.worksheets.length; i++) {
                for (var j = 0; j < testParse.worksheets[i].data.length; j++) {
                    for (var k = 0; k < testParse.worksheets[i].data[j].length; k++) {
                        testParse.worksheets[i].data[j][k].value.should.equal(
                            expectedSheet.worksheets[i].data[j][k]
                        );
                    }
                }
            }
