...
The pipeline starts with raw CSV files containing expression, production rate, and degradation rate data. These files are processed by a preprocessing step that cleans and prepares the data for loading. A loader script then inserts the processed data into the database. All data ultimately populates the `gene_expression` schema.
...
This script (`loader.py`) is used to load your preprocessed expression data into the database.
