...
const buildNetworkSheet = function(genes, links) {
  ...
  // EX: ["CIN5", 0, 0, 1]
  Object.keys(geneNameArray).forEach(
    (index) => (networkSheet[index][0] = geneNameArray[index])
  );
...
const convertToSheet = function(name, testSheet) {
  const singularName = name.toLowerCase().endsWith("s")
    ...
    : name;
  const header = singularName.includes("optimized_")
    ? singularName.substring(10)
    : singularName;
  return {
    name: name,
    data: [
      ["id", header],
      ...Object.keys(testSheet).map((key) => [key, testSheet[key]]),
    ],
  };
};
...
const EXPRESSION_SHEET_SUFFIXES = [
  "_expression",
  "_optimized_expression",
  "_sigmas",
];
...
const buildXlsxSheet = function(workbook) {
  ...
  Object.keys(workbook.exportSheets).forEach((type) => {
    switch (type) {
      ...
      case "expression":
        resultSheet.push(
          ...buildExpressionSheets(workbook.exportSheets.expression)
        );
        break;
      default:
        break;
