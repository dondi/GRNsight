...
var checkDuplicates = function(errorArray, sourceGenes, targetGenes) {
  ...
  for (var i = 0; i < sourceGenes.length - 1; i++) {
    if (sourceGenes[i] === sourceGenes[i + 1]) {
      errorArray.push(
        constants.errors.duplicateGeneError("source", sourceGenes[i])
      );
    }
  }
  ...
  for (var j = 0; j < targetGenes.length - 1; j++) {
    if (targetGenes[j] === targetGenes[j + 1]) {
      errorArray.push(
        constants.errors.duplicateGeneError("target", targetGenes[j])
      );
    }
  }
};
...
var parseNetworkSheet = function(sheet, network) {
  ...
  for (let i = 1; i <= sheet.data[0].slice(1).length; i++) {
    ...
    if (currentGene.name === undefined) {
      ...
      isNaN(currentGene.name) &&
      typeof currentGene.name !== "string"
    ) {
      addWarning(network, constants.warnings.missingSourceGeneWarning(0, i));
    } else {
      // set currentGeneName to a String so toUpperCase doesn't mess up
    ...
    if (sheet.data[row].length === 0) {
      ...
      try {
        ...
        if (sheet.data[row].length < sheet.data[0].length) {
          for (
            let i = sheet.data[row].length - 1;
            i < sheet.data[0].length - 1;
            i++
          ) {
            columnChecker[i]++;
            addWarning(
              network,
              constants.warnings.invalidMatrixDataWarning(row, i)
            );
          }
        }
          ...
          if (row !== 0) {
            ...
            if (column === 0) {
              ...
              try {
                ...
                addError(
                  network,
                  constants.errors.corruptGeneError(row, column)
                );
                return network;
              }
            } else {
              ...
              try {
                if (sheet.data[row][column] === undefined) {
                  ...
                  addError(
                    network,
                    constants.errors.dataTypeError(row, column)
                  );
                  return network;
                } else {
                  ...
                  if (sheet.data[row][column] !== 0) {
                    ...
                    if (sourceGene === undefined || targetGene === undefined) {
                      ...
                      addWarning(
                        network,
                        constants.warnings.randomDataWarning("NaN", row, column)
                      );
                    } else {
                ...
                addError(
                  network,
                  constants.errors.missingValueError(row, column)
                );
  ...
  for (var i = 0; i < columnChecker.length; i++) {
    if (columnChecker[i] >= sheet.data.length - 1) {
      if (sheet.data[0][i + 1] === undefined) {
        ...
        addError(
          network,
          constants.errors.emptyColumnDataError(i + 1, sheet.name)
        );
      }
    }
  }
...
module.exports = function(workbookFile) {
  ...
  for (let i = 0; i < workbookFile.length; i++) {
    ...
    if (workbookFile[i].name.toLowerCase() === "network") {
      ...
      workbookFile[i].name.toLowerCase() === "network_optimized_weights"
    ) {
